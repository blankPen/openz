...
import { DEFAULT_PORT, getDaemonStatePath } from './types.js';
...
export async function startDaemon(port = DEFAULT_PORT) {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('session:create', async (req, ack) => {
            ...
            try {
                ...
                const response = { sessionId: req.id || '', error: err.message };
                ack?.({ error: err.message });
            }
        });
        socket.on('session:send', async (req, ack) => {
            ...
            try {
                ...
                sessionManager.setOnEvent(req.sessionId, (event) => {
                    ...
                    const response = { sessionId: req.sessionId, event };
                    socket.emit('session:event', response);
                });
                // Send message
        ...
        socket.on('session:delete', (req, ack) => {
